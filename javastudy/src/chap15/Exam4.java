...
/*
 * product.txt 파일을 읽어, 6월 그랜저 판매 수량 출력하기
 */
...
public class Exam4 {
	public static void main(String[] args) {
		...
		int sum = br.lines().map(f).filter(c -> c.getMonth() == 6 && c.getCar().equals("그랜저") && c.getCon() == 2)
				.mapToInt(Car::getQty).sum();
		System.out.println(sum);

		//6월 그랜저 판매 레코드 조회
		br = new BufferedReader(new FileReader("product.txt"));
		br.lines().map(f).filter(c -> c.getMonth() == 6 && c.getCar().equals("그랜저") && c.getCon() == 2)
